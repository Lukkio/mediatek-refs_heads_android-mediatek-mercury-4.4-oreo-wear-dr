				 ...
				 enum power_supply_property psp, union power_supply_propval *val)
{
	...
	printk(KERN_ALERT "idtp9220: code: 4\n");

	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = data->WIRELESS_ONLINE;
		printk(KERN_ALERT "idtp9220: code: 5\n");
		break;
	default:
		ret = -EINVAL;
		break;
...
static void mt_battery_update_status(void)
{
...
#else
	{
		...
		printk(KERN_ALERT "idtp9220: code: 11\n");
#endif
...
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	printk(KERN_ALERT "idtp9220: code: 12\n");
#else
...
static void mt_battery_charger_detect_check(void)
{
	if (upmu_is_chr_det() == KAL_TRUE) {
		...
		printk(KERN_ALERT "idtp9220: code: 13\n");

		if ((BMT_status.charger_type == STANDARD_HOST)
		    || (BMT_status.charger_type == CHARGING_HOST)) {
				printk(KERN_ALERT "idtp9220: code: 14\n");
			mt_usb_connect();
		}
#else
		if (BMT_status.charger_type == CHARGER_UNKNOWN) {
...
static int battery_probe(struct platform_device *dev)
{
	...
	printk(KERN_ALERT "idtp9220: code: 15\n");
	if (IS_ERR(wireless_main.psy)) {
		printk(KERN_ALERT "idtp9220: code: 16\n");
		battery_xlog_init_printk(BAT_LOG_CRTI,
