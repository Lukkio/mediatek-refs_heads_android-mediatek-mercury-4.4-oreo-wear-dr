...
static bool CheckNullPointer(void *pointer)
{
    if (pointer == NULL)
    {
        pr_notice("CheckNullPointer pointer = NULL\n");
        return true;
    }
    return false;
}
...
static kal_uint32 AudDrv_SampleRateIndexConvert(kal_uint32 SampleRateIndex)
{
    switch (SampleRateIndex)
    {
        ...
        case 0xa:
            return 48000;
        default:
            pr_notice("AudDrv_SampleRateIndexConvert SampleRateIndex = %d\n", SampleRateIndex);
            return 44100;
    }
    return 0;
}
...
void CheckWriteWaitEvent(void)
{
    ...
    for (i = 0; i < WriteArrayMax; i++)
    {
	/* pr_notice("WriteRecordArray[%d] = %d ",i ,WriteRrecordArray[i]); */
	if (WriteRecordArray[i] > DL1_Interrupt_Interval_ns)
	{
	    OverTimeCounter++;
	}
    }
    /* pr_notice("DL1_Interrupt_Interval_Limit = %d DL1_Interrupt_Interval_ns = %d\n",DL1_Interrupt_Interval_Limit,DL1_Interrupt_Interval_ns); */
...
static int AudDrv_Read_Procmem(struct seq_file *buf, void *v)
{
    ...
    pr_notice("AudDrv_Read_Procmem len = %d\n", len);
...
void Auddrv_Handle_Mem_context(AFE_MEM_CONTROL_T *Mem_Block)
{
    ...
    {
        pr_notice("[Auddrv] Mem_Block ==[NULL] || AudDrvSuspendStatus = [%d]\n", AudDrvSuspendStatus);
        return;
    }
    ...
    {
        pr_notice("AudDrv: HW_Cur_ReadIdx 0");
        return;
    }
    ...
    {
        pr_notice("AudDrv: Auddrv_Handle_Mem_context pucVirtBufAddr NULL");
        return;
    }
    ...
    {
        pr_notice("AudDrv=>+UL_Handling overflow ");
        pr_notice("AudDrv=>DataRemained:0x%x R:0x%x W:0x%x BufSize:0x%x Hw_Get_bytes:0x%x ", mBlock->u4DataRemained, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx, mBlock->u4BufferSize, Hw_Get_bytes);
        pr_notice("AudDrv=>HW_Cur_ReadIdx:0x%x pucPhysBufAddr:0x%x", HW_Cur_ReadIdx, mBlock->pucPhysBufAddr);
...
void Auddrv_DL_Interrupt_Handler(void)  /* irq1 ISR handler */
{
    ...
    {
        // handle DL1 Context
        pr_notice("%s  AFE_dL1_Control_context\n",__func__);
    ...
    {
        // handle DL2 Context
        pr_notice("%s  AFE_dL2_Control_context\n",__func__);
...
void Auddrv_Handle_DL_Mem_context(AFE_MEM_CONTROL_T *Mem_Block)
{
    ...
    pr_notice("%s pAfe_MEM_ConTrol->MemIfNum = %d Mem_Block->MemIfNum = %d\n ", __func__,pAfe_MEM_ConTrol->MemIfNum,Mem_Block->MemIfNum);

    if (pAfe_MEM_ConTrol == NULL)
    {
        pr_notice("cannot find MEM control !!!!!!!\n");
        return;
    }

    if (Afe_Block->u4BufferSize <= 0)
    {
        pr_notice("Afe_Block->u4BufferSize \n");
        return;
    }
    ...
    else
    {
        ...
        switch (pAfe_MEM_ConTrol->MemIfNum)
        {
            ...
            default:
                pr_notice("Auddrv_Handle_DL_Mem_context cannot find match MemIfNum!!!\n");
                HW_Cur_ReadIdx =0;
        }
    }
    /* if(HW_Cur_ReadIdx == 0) */
    ...
    pr_notice("[Auddrv] HW_Cur_ReadIdx=0x%x HW_memory_index = 0x%x Afe_Block->pucPhysBufAddr = 0x%x\n",
    ...
    if ((Afe_consumed_bytes & 0x3) != 0) {
        pr_notice("[Auddrv] MEMIF address is not aligned 4 bytes\n");
    }
    ...
    if (Afe_Block->u4DataRemained < Afe_consumed_bytes || Afe_Block->u4DataRemained <= 0 || Afe_Block->u4DataRemained  > Afe_Block->u4BufferSize || AudIrqReset)
    {
        ...
        pr_notice("AudDrv=>+DL_Handling underflow ");
        pr_notice("AudDrv=>DataRemained:0x%x R:0x%x W:0x%x BufSize:0x%x Consumed_bytes:0x%x ", Afe_Block->u4DataRemained, Afe_Block->u4DMAReadIdx, Afe_Block->u4WriteIdx, Afe_Block->u4BufferSize, Afe_consumed_bytes);
        pr_notice("AudDrv=>HW_memory_index:0x%x pucPhysBufAddr:0x%x AudIrqReset:0x%x", HW_memory_index, Afe_Block->pucPhysBufAddr, AudIrqReset);
    ...
    {
        pr_notice("+DL_Handling normal ReadIdx:%x ,DataRemained:%x, WriteIdx:%x\n",
    ...
    pr_notice("Auddrv_Handle_DL_Mem_context_+lock1");
    pr_notice("Auddrv_Handle_DL_Mem_context_-lock1");
    /* wait up write thread */
    switch (pAfe_MEM_ConTrol->MemIfNum)
    {
        ...
        case MEM_DL2:
        {
            ...
            pr_notice("Auddrv_Handle_DL_Mem_context_+lock2");
            pr_notice("Auddrv_Handle_DL_Mem_context_-lock2");
...
static void CheckInterruptTiming(void)
{
	...
	if ((Irq_time_t1 > Irq_time_t2) && DL1_Interrupt_Interval_Limit)
	{
	    ...
	    {
		pr_notice("CheckInterruptTiming interrupt may be blocked Irq_time_t2 = %llu DL1_Interrupt_Interval_Limit = %d\n",
...
static irqreturn_t AudDrv_IRQ_handler(int irq, void *dev_id)
{
    ...
    u4RegValue &= 0xf;
    u4RegValue2 &= 0x3;
    pr_notice("AudDrv_IRQ_handler AFE_IRQ_MCU_STATUS = %x\n",u4RegValue);
    ...
    {
	pr_notice("AudDrv u4RegValue == %d Aud_AFE_Clk_cntr = %d u4RegValue2 = %d\n", u4RegValue, Aud_AFE_Clk_cntr, u4RegValue2);
	AudioWayDisable();
	/*
	AudDrv_Clk_On();
	...
	*/
	...
	pr_notice("IRQ Handle Exception");
...
static void AudDrv_Store_reg_AFE(void)
{
   ...
   AudDrv_Clk_Off();
   pr_notice("-AudDrv_Store_reg\n");
}

static void AudDrv_Recover_reg_AFE(void)
{
    pr_notice(KERN_EMERG "+AudDrv_resume");
    AudDrv_Clk_On();
...
/* only one suspend mode */
{
    ...
    {
	pr_notice("AudDrv_suspend: b_modem1_speech_on(%d), return", b_modem1_speech_on);
	return 0;
    }

    pr_notice("AudDrv_suspend AudDrvSuspendStatus = %d bSpeechFlag = %d\n",AudDrvSuspendStatus,SPH_Ctrl_State.bSpeechFlag);
...
void CheckPowerState(void)
{
	...
	if(Reg_clksq_en ==0)
	{
	    pr_notice("CheckPowerState Reg_clksq_en = 0x%x\n",Ana_Get_Reg (TOP_CKCON1));
	} */
}
...
static int AudDrv_resume(struct platform_device *dev) /* wake up */
{
   /* PRINTK_AUDDRV("+AudDrv_resume AudDrvSuspendStatus= %d\n",AudDrvSuspendStatus); */
	...
	AudDrv_Clk_On();
	pr_notice("+AudDrv resume memset zero for sysram");
	memset(Afe_Block->pucVirtBufAddr, 0, Afe_Block->u4BufferSize);
	pr_notice("+AudDrv resume memset zero for sysram");
	AudDrv_Clk_Off();
...
{
    pr_notice("AudDrv_release inode:%p, file:%p\n", inode, fp);
...
void Auddrv_Free_Dma_Memory(AFE_MEM_CONTROL_T *pAFE_MEM)
{
    ...
    if (pAFE_MEM == NULL)
    {
	pr_notice("Auddrv_Free_Dma_Memory pAFE_MEM = NULL");
    }

    pblock =  &(pAFE_MEM->rBlock);
    if ((pblock->pucVirtBufAddr != NULL) && (pblock->pucPhysBufAddr != 0))
    {
	pr_notice("dma_free_coherent pucVirtBufAddr = %p pucPhysBufAddr = %x", pblock->pucVirtBufAddr, pblock->pucPhysBufAddr);
	dma_free_coherent(0, pblock->u4BufferSize, pblock->pucVirtBufAddr, pblock->pucPhysBufAddr);
    }
    else
    {
	pr_notice("cannot dma_free_coherent pucVirtBufAddr = %p pucPhysBufAddr = %x", pblock->pucVirtBufAddr, pblock->pucPhysBufAddr);
    }
}
...
int AudDrv_Force_Free_DL1_Buffer(void)
{
    ...
    pr_notice("+AudDrv_Force_Free_DL1_Buffer\n");
	...
	if ((pblock->pucVirtBufAddr != NULL) && (pblock->pucPhysBufAddr != 0))
	{
	    pr_notice("AudDrv_Force_Free_DL1_Buffer pucVirtBufAddr = %p pucPhysBufAddr = %x", pblock->pucVirtBufAddr, pblock->pucPhysBufAddr);
	    dma_free_coherent(0, pblock->u4BufferSize, pblock->pucVirtBufAddr, pblock->pucPhysBufAddr);
	}
	else
	{
	    pr_notice("cannot AudDrv_Force_Free_DL1_Buffer pucVirtBufAddr = %p pucPhysBufAddr = %x", pblock->pucVirtBufAddr, pblock->pucPhysBufAddr);
	}
#endif
    ...
    pr_notice("-AudDrv_Free_DL1_Buffer\n");
    return 0;
}
...
int AudDrv_Force_Free_Buffer(int mem_type)
{
    ...
    AFE_BLOCK_T *pblock = NULL;
    pr_notice("+ AudDrv_Force_Free_Buffer mem_type = %d\n", mem_type);
    switch (mem_type)
    {
	case MEM_DL1:
	{
    ...
    pr_notice("-AudDrv_Force_Free_Buffer\n");
    return 0;
}
...
int AudDrv_Allocate_DL1_Buffer(struct file *fp, kal_uint32 Afe_Buf_Length)
{
    ...
    pr_notice("AudDrv_Allocate_DL1_Buffer length fp = %p = %d\n", fp, Afe_Buf_Length);
...
#ifdef AUDIO_MEM_IOREMAP
	pr_notice("AudDrv_Allocate_DL1_Buffer length AUDIO_MEM_IOREMAP = %p = %d\n", fp, Afe_Buf_Length);
...
#else
	pr_notice("AudDrv_Allocate_DL1_Buffer use dram");
    ...
    pr_notice("AudDrv_Allocate_DL1_Buffer pucVirtBufAddr = %p\n", pblock->pucVirtBufAddr);

    /* check 32 bytes align */
    if ((pblock->pucPhysBufAddr & 0x1f) != 0)
    {
	pr_notice("[Auddrv] AudDrv_Allocate_DL1_Buffer is not aligned (0x%x)\n", pblock->pucPhysBufAddr);
    }
...
{
    AFE_BLOCK_T *pblock = &AFE_dL1_Control_context.rBlock;
    pr_notice("+AudDrv_Free_DL1_Buffer fp = %p", fp);
    if (pblock->pucVirtBufAddr != NULL)
    {
	/* todo:: here need to free sram by sram manager */
...
int AudDrv_Allocate_Buffer(struct file *fp, kal_uint32 Afe_Buf_Length , int mem_type)
{
    AFE_MEM_CONTROL_T *pAFE_MEM = NULL;
    AFE_BLOCK_T *pblock = NULL;
    pr_notice("AudDrv_Allocate_Buffer fp = %p length = %d mem_type = %d\n", fp, Afe_Buf_Length, mem_type);
    switch (mem_type)
    {
	case MEM_DL1:
	{
	...
	{
	    pr_notice("AudDrv_Allocate_Buffer dma_alloc_coherent fail\n");
	    return -1;
	}
...
#endif
	pr_notice("pblock->pucVirtBufAddr = %p pblock->pucPhysBufAddr = 0x%x\n mem_type = %d\n" ,
...
int AudDrv_Free_Buffer(struct file *fp, int mem_type)
{
    AFE_MEM_CONTROL_T *pAFE_MEM = NULL;
    AFE_BLOCK_T *pblock = NULL;
    pr_notice("+ AudDrv_Free_Buffer fp = %p mem_type = %d\n", fp, mem_type);
    switch (mem_type)
    {
	case MEM_DL1:
	{
...
void Auddrv_Add_MemIF_Counter(int MEM_Type)
{
	...
	case MEM_DL1:
		...
		pr_notice("+ AudDrv Add_MemIF_Counter memset 0");
		memset(Afe_Block->pucVirtBufAddr, 0, Afe_Block->u4BufferSize);
		pr_notice("- AudDrv Add_MemIF_Counter memset 0");
	    }
	    break;
	...
	case MEM_MOD_DAI:
	    break;
	default:
	    pr_notice("Auddrv_Add_MemIF_Conter MEMTYPE = %d", MEM_Type);
	    return;
    }
}
...
void Auddrv_Release_MemIF_Counter(int MEM_Type)
{
	...
	case MEM_DL1:
	    ...
	    if (Afe_Mem_Pwr_on < 0)
	    {
		pr_notice("Auddrv_Release_MemIF_Conter Afe_Mem_Pwr_on <0\n");
		Afe_Mem_Pwr_on = 0;
	    }
	    ResetWriteWaitEvent();
	    break;
	case MEM_DL2:
        ResetWriteWaitEvent();
		pr_notice("DL2 Auddrv_Release_MemIF_Conter\n");
	    break;
	case MEM_AWB:
	    break;
	case MEM_VUL:
...
int AudDrv_Reassign_Buffer_In_SRAM(struct file *fp, unsigned long arg)
{
    ...
    pr_notice("AudDrv_Reassign_Buffer_In_SRAM type = %d, length %d fp = %p\n", mem_type, AFE_Buffer_Size, fp);
...
#ifdef AUDIO_MEM_IOREMAP
    pblock->pucVirtBufAddr = (kal_uint8 *)AFE_SRAM_ADDRESS;
    pr_notice("AudDrv_Reassign_Buffer_In_SRAM AUDIO_MEM_IOREMAP = %p, length = %d, addr 0x%p\n", fp, AFE_Buffer_Size, pblock->pucVirtBufAddr);
#else
    pblock->pucVirtBufAddr = AFE_INTERNAL_SRAM_VIR_BASE;
#endif

    pr_notice("AudDrv_Reassign_Buffer_In_SRAM pucVirtBufAddr = %p\n", pblock->pucVirtBufAddr);
    ...
    pr_notice("-AudDrv_Reassign_Buffer_In_SRAM\n");
    return 0;
}
...
void Auddrv_Set_MemIF_Fp(struct file *fp, unsigned long arg)
{
    ...
    pr_notice("+Auddrv_Set_MemIF_Fp  = %p arg = %lu\n", fp , arg);
    if (pAfe_MEM_ConTrol == NULL)
    {
	pr_notice("+ pAfe_MEM_ConTrol Error !!! pAfe_MEM_ConTrol = NULL, fp = %p,\n", fp);
	return;
    }
    else if (pAfe_MEM_ConTrol->flip != NULL)
    {
	pr_notice("+ pAfe_MEM_ConTrol flip = %p Error fp = %p\n", pAfe_MEM_ConTrol->flip, fp);
	return;
    }
...
void Auddrv_Check_Irq(void)
{
    ...
    if (u4RegValue & 0x1)
    {
	pr_notice("Auddrv_Check_Irq u4RegValue == %d\n", u4RegValue);
	Afe_Set_Reg(AFE_IRQ_CLR, 1 , 0xff);
    }
...
void Auddrv_Release_MemIF_Fp(struct file *fp, unsigned long arg)
{
    AFE_MEM_CONTROL_T *pAfe_MEM_ConTrol;
    pAfe_MEM_ConTrol = Auddrv_Get_MemIF_Context(arg);
    pr_notice("+Auddrv_Release_MemIF_Fp  = %p arg = %lu\n", fp , arg);
    if (pAfe_MEM_ConTrol == NULL)
    {
	pr_notice("+ pAfe_MEM_ConTrol Error !!! pAfe_MEM_ConTrol = NULL, fp = %p,\n", fp);
	return;
...
static long AudDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    pr_notice("AudDrv_ioctl cmd = %u arg = %lu\n", cmd, arg);
    switch (cmd)
    {
	case SET_AUDSYS_REG:
	{
	...
	case AUD_RESTART:
	{
	    pr_notice("AudDrv AUD_RESTART cmd = %x\n", cmd);
	    ...
	    {
		unsigned long flags = 0;
		pr_notice("AudDrv +Before AudDrv_Clk_On_DisableISR Clk lock\n");
		spin_lock_irqsave(&auddrv_clk_isr_lock, flags);
		pr_notice("AudDrv +Force DisableISR\n");
		AudDrv_Clk_On_DisableISR();
		pr_notice("AudDrv -Force DisableISR\n");
		spin_unlock_irqrestore(&auddrv_clk_isr_lock, flags);
		pr_notice("AudDrv -After AudDrv_Clk_On_DisableISR Clk lock\n");

		spin_lock(&auddrv_lock);
		if (Aud_AFE_Clk_cntr > 0)
		{
		    unsigned long flags_ = 0;
		    pr_notice("AudDrv +Before Force Disable Clk lock\n");
		    spin_lock_irqsave(&auddrv_clk_isr_lock, flags_);
		    pr_notice("AudDrv +Force Disable Clk\n");
		    AudDrv_Clk_Off();
		    pr_notice("AudDrv -Force Disable Clk\n");
		    spin_unlock_irqrestore(&auddrv_clk_isr_lock, flags_);
		    pr_notice("AudDrv -After Force Disable Clk lock\n");
		    Aud_AFE_Clk_cntr = 0;
		    Aud_Core_Clk_cntr = 0;
		    Afe_Mem_Pwr_on = 0;
		}
	...
	case AUDDRV_RESET_BT_FM_GPIO:
	{
	    pr_notice("!! NoCombo, COMBO_AUDIO_STATE_0\n");
	    break;
	}
	case AUDDRV_SET_BT_PCM_GPIO:
	{
	    pr_notice("!! NoCombo, COMBO_AUDIO_STATE_1\n");
	    break;
	}
	case AUDDRV_SET_FM_I2S_GPIO:
	{
	...
	case SET_AUDIO_STATE:
	{
	    ...
	    pr_notice("AudDrv SET_AUDIO_STATE\n");
	...
	{
	    pr_notice("AudDrv AUDDRV_LOG_PRINT\n");
	    pr_notice("Afe_Mem_Pwr_on =0x%x\n", Afe_Mem_Pwr_on);
	    pr_notice("Aud_AFE_Clk_cntr = 0x%x\n", Aud_AFE_Clk_cntr);
	    pr_notice("Aud_ANA_Clk_cntr = 0x%x\n", Aud_ANA_Clk_cntr);
	    pr_notice("Aud_HDMI_Clk_cntr = 0x%x\n", Aud_HDMI_Clk_cntr);
	    pr_notice("Aud_I2S_Clk_cntr = 0x%x\n", Aud_I2S_Clk_cntr);
	    pr_notice("AuddrvSpkStatus = 0x%x\n", AuddrvSpkStatus);
	    Ana_Log_Print();
	    Afe_Log_Print();
	    break;
	}
	...
	default:
	{
	    pr_notice("AudDrv Fail IOCTL command no such ioctl cmd = %x\n", cmd);
	    ret = -1;
	    break;
	}
    }
...
static ssize_t AudDrv_write(struct file *fp, const char __user *data, size_t count, loff_t *offset)
{
    ...
    pr_notice("+AudDrv_writeAudDrv_write = %p count = %d\n",fp ,count);
    ...
    {
	pr_notice("AudDrv_writeAudDrv_write g fbut find no MEM control block");
	msleep(60);
	return written_size;
    }
    ...
    if (Afe_Block->u4BufferSize == 0)
    {
	pr_notice("AudDrv_write: u4BufferSize=0 Error");
	    ...
	    else
	{
	    ...
	    pr_notice("AudDrv_write copy_size:%x\n", copy_size);  // (free  space of buffer)
	}
	...
	if (copy_size != 0)
	{
	   ...
	   if (Afe_WriteIdx_tmp > Afe_Block->u4BufferSize) {
	       pr_notice("[AudDrv][Warn]mcmcpy Afe_Block->pucVirtBufAddr=%p, Afe_WriteIdx= %x data_w_ptr = %p copy_size = %x\n",
		...
		{
		    pr_notice("AudDrv_write 0ptr invalid data_w_ptr=%x, size=%d", (kal_uint32)data_w_ptr, copy_size);
		    pr_notice("AudDrv_write u4BufferSize=%d, u4DataRemained=%d", Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
		}
		else
		{
		    //pr_notice("mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %x data_w_ptr = %p copy_size = %zu\n",
			//Afe_Block->pucVirtBufAddr+Afe_WriteIdx_tmp,data_w_ptr,copy_size);
		...
		count -= copy_size;
		pr_notice("AudDrv_write finish1, copy_size:%x, WriteIdx:%x, ReadIdx=%x, DataRemained:%x, count=%d \r\n",
		...
		{
		    pr_notice("AudDrv_write 1ptr invalid data_w_ptr=%x, size_1=%d", (kal_uint32)data_w_ptr, size_1);
		    pr_notice("AudDrv_write u4BufferSize=%d, u4DataRemained=%d", Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
		}
		else
		{
		    //pr_notice("mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %x data_w_ptr = %p size_1 = %zu\n",
		...
		{
		    pr_notice("AudDrv_write 2ptr invalid data_w_ptr=%x, size_1=%d, size_2=%d", (kal_uint32)data_w_ptr, size_1, size_2);
		    pr_notice("AudDrv_write u4BufferSize=%d, u4DataRemained=%d", Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
		}
		else
		{
		    //pr_notice("mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %x data_w_ptr+size_1 = %p size_2 = %zu\n",
		...
		count -= copy_size;
		data_w_ptr += copy_size;
		pr_notice("AudDrv_write finish2, copy size:%x, WriteIdx:%x,ReadIdx=%x DataRemained:%x \r\n",
	...
	{
	    pr_notice("AudDrv_write copy_size =0, copy size:%x, WriteIdx:%x,ReadIdx=%x DataRemained:%x \r\n",
		copy_size,Afe_Block->u4WriteIdx,Afe_Block->u4DMAReadIdx,Afe_Block->u4DataRemained );
	}
	...
	if (count != 0)
	{
	    ...
	    pr_notice("AudDrv_write wait for interrupt count=%x\n",count);
	    ...
	    {
		pr_notice("AudDrv_write timeout, [Warning]blocked by others.(%llu)ns,(%d)jiffies written_size = %d\n",
			      t2, (DL1_Interrupt_Interval * 2 / 10), written_size);
		pr_notice("auddrv write t2 = %llu t1 = %llu t2-t1 = %llu WriteArrayIndex = %d\n", t2, t1, t2 - t1, WriteArrayIndex);
...
ssize_t AudDrv_MEMIF_Read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
    ...
    if (!Auddrv_CheckRead_MemIF_Fp(pAfe_MEM_ConTrol->MemIfNum))
    {
	pr_notice("cannot find matcg MemIfNum!!!\n");
	msleep(50);
	return -1;
    }
    ...
    /* handle for buffer management */
   pr_notice("AudDrv_MEMIF_Read WriteIdx=%x, ReadIdx=%x, DataRemained=%x\n",
	Afe_Block->u4WriteIdx, Afe_Block->u4DMAReadIdx,Afe_Block->u4DataRemained);
    while (count)
    {
	...
	{
	    pr_notice("AudDrv_MEMIF_Read u4DataRemained=%x > u4BufferSize=%x" , Afe_Block->u4DataRemained, Afe_Block->u4BufferSize);
	...
	pr_notice("AudDrv_MEMIF_Read finish0, read_count:%x, read_size:%x, u4DataRemained:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x \r\n",
	...
	if (DMA_Read_Ptr + read_size < Afe_Block->u4BufferSize)
	{
	    ...
	    {
		pr_notice("AudDrv_MEMIF_Read 1, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
	    ...
	    {
		pr_notice("AudDrv_MEMIF_Read E1, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x pucVirtBufAddr : %x \r\n",
	    ...
	    {
		pr_notice("AudDrv_MEMIF_Read Fail 1 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%x, read_size:%x",
	    ...
	    count -= read_size;
	    pr_notice("AudDrv_MEMIF_Read finish1, copy size:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x, u4DataRemained:%x \r\n",
	...
	else
	{
	    ...
	    if (DMA_Read_Ptr != Afe_Block->u4DMAReadIdx)
	    {
		pr_notice("AudDrv_MEMIF_Read 2, read_size1:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
		     size_1,Afe_Block->u4DataRemained,DMA_Read_Ptr,Afe_Block->u4DMAReadIdx);
	    }

            if (DMA_Read_Ptr < 0 || DMA_Read_Ptr > Afe_Block->u4BufferSize)
	    {
		pr_notice("AudDrv_MEMIF_Read E2, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x pucVirtBufAddr : %x \r\n",
			      read_size, Afe_Block->u4DataRemained, DMA_Read_Ptr, Afe_Block->u4DMAReadIdx, (unsigned int)Afe_Block->pucVirtBufAddr);
	    }

	    if (copy_to_user((void __user *)Read_Data_Ptr, (Afe_Block->pucVirtBufAddr + DMA_Read_Ptr), size_1))
	    {
		pr_notice("AudDrv_MEMIF_Read Fail 2 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%x, read_size:%x",
		     Read_Data_Ptr,Afe_Block->pucVirtBufAddr, Afe_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
		if (read_count == 0)
		{
		    return -1;
	    ...
	    DMA_Read_Ptr = Afe_Block->u4DMAReadIdx;
	    spin_unlock_irqrestore(&auddrv_ULInCtl_lock, flags);
	    pr_notice("AudDrv_MEMIF_Read finish2, copy size_1:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, u4DataRemained:%x \r\n",
		...
		xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_AWB_Read 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
            ...
            if (DMA_Read_Ptr < 0 || DMA_Read_Ptr > Afe_Block->u4BufferSize)
	    {
		pr_notice("AudDrv_MEMIF_Read E3, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x pucVirtBufAddr : %x \r\n",
			      read_size, Afe_Block->u4DataRemained, DMA_Read_Ptr, Afe_Block->u4DMAReadIdx, (unsigned int)Afe_Block->pucVirtBufAddr);
	    }
	    ...
	    count -= read_size;
	    Read_Data_Ptr += read_size;
	 pr_notice("AudDrv_MEMIF_Read finish3, copy size_2:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x u4DataRemained:%x \r\n",
		size_2,Afe_Block->u4DMAReadIdx,Afe_Block->u4WriteIdx,Afe_Block->u4DataRemained );
	}

	if (count != 0)
	{
	   pr_notice("AudDrv_MEMIF_Read wait for interrupt signal pAfe_MEM_ConTrol->MemIfNum = %d\n",pAfe_MEM_ConTrol->MemIfNum);
	    switch (pAfe_MEM_ConTrol->MemIfNum)
	    {
		case MEM_VUL:
		{
