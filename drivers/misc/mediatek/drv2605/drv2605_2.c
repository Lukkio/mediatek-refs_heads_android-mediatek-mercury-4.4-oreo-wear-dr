...
static struct drv260x_platform_data *drv260x_of_init(struct i2c_client *client)
{
	...
	*/
	...
	pdata->en_gpio = 1;
	pdata->trigger_gpio = 1;
	pdata->external_trigger = 1;
	pdata->default_effect = 17;
	pdata->rated_voltage = 0x90;
	pdata->overdrive_voltage = 0x90;
	pdata->rtp_overdrive_voltage = 0x90;
	pdata->effects_library = 0xff;
	pdata->disable_calibration = 0;
	pdata->vibrator_vdd = 0;
	pdata->static_vdd = 0;
	regs_arr = 0;
						...
						unsigned int size)
{
	...
	/*drv260x->client->addr = 0x5a;
	drv260x->client->timing = 400;
	

	dev_err(&drv260x->client->dev, "drv2605: TIMING %d\n", drv260x->client->timing);
	dev_err(&drv260x->client->dev, "drv2605: ADDR %d\n", drv260x->client->addr);
	dev_err(&drv260x->client->dev, "drv2605: flags %d\n", drv260x->client->flags);
	dev_err(&drv260x->client->dev, "drv2605: ext_flag %d\n", drv260x->client->ext_flag);

	drv260x->client->ext_flag = 1;
	dev_err(&drv260x->client->dev, "drv2605: ext_flag %d\n", drv260x->client->ext_flag);
	*/
	for (tries = 0; tries < I2C_RETRIES; tries++) {
		//ret = i2c_master_send(drv260x->client, data, sizeof(data));

		ret = i2c_smbus_write_byte_data(drv260x->client, WAVEFORM_SEQUENCER_REG, WAVEFORM_SEQUENCER_DEFAULT);
...
static void drv260x_change_mode(char mode)
{
	...
	if (drv260x->external_trigger) hwPowerOn(MT6323_POWER_LDO_VIBR, vibr_voltage, "VIBR");
...
static int drv260x_init(void)
{
	...
	printk(KERN_ALERT "drv260x: initialized test\n");
	return 0;
